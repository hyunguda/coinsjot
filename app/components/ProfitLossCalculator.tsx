...
const inputCls =
  "w-full px-2.5 py-2 bg-gray-50 border border-transparent rounded-lg text-sm focus:bg-white focus:ring-2 focus:ring-blue-400 outline-none transition tabular-nums";
...
export function ProfitLossCalculator() {
  ...
  const ActionButtons = () => (
    <div className="flex gap-2">
      <button onClick={addRow} className="px-4 py-2 bg-blue-50 text-blue-600 rounded-lg text-sm font-medium hover:bg-blue-100 transition">
        + 종목 추가
      </button>
      <button onClick={reset} className="px-4 py-2 bg-gray-50 text-gray-500 rounded-lg text-sm font-medium hover:bg-gray-100 transition">
        초기화
      </button>
    </div>
  );
    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="sm:hidden px-4 pt-4 pb-2 space-y-3">
          {parsed.map((row, i) => (
            <div key={row.id} className="bg-gray-50 rounded-xl p-4">
              {/* 상단: 번호 + 코인명 + 삭제 */}
              <div className="flex items-center gap-2 mb-3">
                ...
                <input
                  ...
                  className="flex-1 px-3 py-2 bg-white border border-gray-100 rounded-lg text-sm focus:ring-2 focus:ring-blue-400 outline-none transition"
                />
                <button
                  onClick={() => removeRow(row.id)}
                  disabled={rows.length <= 1}
                  className="w-8 h-8 flex items-center justify-center text-gray-300 hover:text-red-400 disabled:cursor-not-allowed text-lg flex-shrink-0"
                >×</button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-3">
                <div>
                  <p className="text-[10px] text-gray-400 mb-1 ml-1">매수가 (원)</p>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={fmtIn(row.buyPrice)}
                    placeholder="0"
                    onChange={(e) => update(row.id, "buyPrice", parseIn(e.target.value))}
                    className="w-full px-3 py-2 bg-white border border-gray-100 rounded-lg text-sm text-right tabular-nums focus:ring-2 focus:ring-blue-400 outline-none transition"
                  />
                </div>
                <div>
                  <p className="text-[10px] text-gray-400 mb-1 ml-1">매도가 (원)</p>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={fmtIn(row.sellPrice)}
                    placeholder="0"
              ...
              {row.pnl !== null && (
                <div className="flex justify-between items-center pt-2.5 border-t border-gray-100">
                  <span className={`text-sm font-bold tabular-nums ${row.pnl >= 0 ? "text-blue-600" : "text-red-500"}`}>
                    {fmtSigned(row.pnl)}
                  </span>
                  <span className={`text-sm font-bold tabular-nums ${(row.rate as number) >= 0 ? "text-blue-600" : "text-red-500"}`}>
                    {fmtRate(row.rate as number)}
                  </span>
                </div>
              )}
            </div>
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              <p className="text-xs text-gray-400 mb-2">총 익절 합계</p>
              <p className="text-base font-bold tabular-nums text-blue-600 leading-snug">
                +{s.totalGain.toLocaleString("ko-KR")}
              </p>
              <p className="text-xs text-gray-400">원</p>
            </div>
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              <p className="text-xs text-gray-400 mb-2">총 손절 합계</p>
              <p className="text-base font-bold tabular-nums text-red-500 leading-snug">
                {s.totalLoss.toLocaleString("ko-KR")}
              </p>
              <p className="text-xs text-gray-400">원</p>
            </div>
            <div className="col-span-2 sm:col-span-1 bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4">
              <p className="text-xs text-gray-400 mb-2">손익통산 합계</p>
              <p className={`text-base font-bold tabular-nums leading-snug ${s.netPnl >= 0 ? "text-green-600" : "text-orange-500"}`}>
                {fmtSigned(s.netPnl)}
              </p>
              {s.rateB !== null && (
                <p className={`text-xs font-semibold tabular-nums mt-0.5 ${s.rateB >= 0 ? "text-green-400" : "text-orange-400"}`}>
                  {fmtRate(s.rateB)}
                </p>
              )}
            </div>
            ...
            <div className="p-5">
              <div className="rounded-xl border border-gray-100 divide-y divide-gray-100 overflow-hidden">
                <div className="px-5 py-4 flex items-center justify-between gap-3 bg-gray-50">
                  <div className="flex items-center gap-2 min-w-0">
                    <Tag text="F = B − E" />
                    <span className="text-sm text-gray-600 truncate">세후 최종 손익</span>
                  </div>
                  <span className={`text-base font-bold tabular-nums flex-shrink-0 ${s.netAfter >= 0 ? "text-blue-600" : "text-red-500"}`}>
                ...
                <div className="px-5 py-4 flex items-center justify-between gap-3 bg-gray-50">
                  <div className="flex items-center gap-2 min-w-0">
                    <Tag text="B ÷ A" />
                    <span className="text-sm text-gray-500 truncate">세전 수익률</span>
                  </div>
                  <span className={`text-sm font-semibold tabular-nums flex-shrink-0 ${s.rateB !== null && s.rateB >= 0 ? "text-blue-600" : "text-red-500"}`}>
                    {s.rateB !== null ? fmtRate(s.rateB) : "—"}
                  </span>
                </div>
                <div className="px-5 py-5 flex items-center justify-between gap-3 bg-white">
                  <div className="flex items-center gap-2 min-w-0">
                    <Tag text="F ÷ A" />
                    <span className="text-sm font-semibold text-gray-800 truncate">세후 수익률</span>
                  </div>
                  <span className={`text-2xl font-bold tabular-nums flex-shrink-0 ${s.rateF !== null && s.rateF >= 0 ? "text-green-600" : "text-red-500"}`}>
